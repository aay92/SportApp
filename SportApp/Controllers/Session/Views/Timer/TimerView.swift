...
//        длительность и прогресс в секундах
    ...
    public func startTimer(){
       timer.invalidate()
// @escaping помечая параметр мы говорим что ,когда функция закончит свое выполнение и выгрузится из памяти completion сохраняет ссылку и будет держать результат этой функции
        timer = Timer.scheduledTimer(withTimeInterval: 0.01,
            ...
            if self.timerProgress > self.timerDuration {
                ...
                print("timer : \(self.timerProgress)")
                timer.invalidate()
//                self.callBack?()
            }
            print("timer print: \(self.timerProgress)")
